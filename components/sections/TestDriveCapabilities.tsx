...
export default async function TestDriveCapabilities() {
  const { testDrive } = await getDictionary();
  const capabilities = testDrive.capabilities;
    ...
    <section aria-labelledby="test-drive-capabilities-heading" className="flex justify-center px-5 py-10 sm:py-16">
      <div className="flex w-full max-w-[1400px] flex-col items-center gap-10">
        <h2 id="test-drive-capabilities-heading" className="text-center text-[clamp(1.25rem,0.4167vw+1.1667rem,1.5rem)] leading-[1.2] font-bold text-texto">
          ...
          <span className="text-azul-base">{capabilities.headingHighlight}</span>
        </h2>

        <Reveal className="w-full">
          <ul className="grid w-full grid-cols-2 gap-4 sm:grid-cols-4 lg:grid-cols-8">
            {testDriveCapabilities.map((item, index) => {
              const text = capabilities.items[index];
              return (
                <li
                  key={text.title}
                ...
                >
                  ...
                  <div className="flex flex-col gap-[15px]">
                    <p className={`text-[clamp(1.0625rem,0.1042vw+1.0417rem,1.125rem)] font-bold leading-[1.2] ${item.titleClassName}`}>{text.title}</p>
                    <p className="text-base leading-[1.2] font-medium text-texto-medio">{text.description}</p>
                  </div>
                </li>
              );
            })}
