...
export default async function ParceriaHowItWorks() {
  ...
  const cardBasis = getCompleteBoxBasis(steps.length);
  ...
  return (
    <section aria-labelledby="parceria-how-heading" className="flex justify-center bg-branco px-5 py-10 sm:py-16">
      <div className="flex w-full max-w-[1400px] flex-col items-center gap-10">
        ...
        {/* Below lg: "Blocos Mobile" — each step becomes its own bordered
            card via "Complete Box" (see lib/completeBox.ts). The step
            number badge is kept since it's essential to a numbered list. */}
        <ol className="flex w-full flex-wrap gap-4 lg:hidden">
          {steps.map((step, i) => (
            <Reveal key={step.title} delayMs={i * 120} className={`grow min-w-[160px] ${cardBasis}`}>
              <li className="relative flex h-full min-h-[170px] flex-col items-center justify-center gap-2.5 rounded-[14px] border border-contorno-base bg-branco p-5 text-center">
                <Image src={step.icon} alt="" aria-hidden="true" width={30} height={30} />
                <div className="flex w-full flex-col items-center gap-2">
                  ...
                  <p className="w-full text-sm leading-[1.2] font-medium text-texto">{step.description}</p>
                </div>
        ...
        <ol className="hidden w-full lg:grid lg:grid-cols-4 lg:gap-5">
          {steps.map((step, i) => (
