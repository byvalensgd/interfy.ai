...
export default async function PlatformOverview() {
  const { home, platform } = await getDictionary();
  const { overview } = platform;

  const stats = platformStatIcons.map((stat, i) => ({ icon: stat.icon, ...home.platformIntro.stats[i] }));
  ...
  return (
    ...
    >
      <div className="flex w-full max-w-[1400px] flex-col items-center gap-10">
        ...
        <Reveal delayMs={200}>
          <StatsBar stats={stats} label={home.platformIntro.statsAriaLabel} size="lg" />
        </Reveal>
      </div>
    </section>
  );
