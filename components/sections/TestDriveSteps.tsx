...
export default async function TestDriveSteps() {
  const { testDrive } = await getDictionary();
  const steps = testDrive.steps;
    ...
    >
      <div className="flex w-full max-w-[1400px] flex-col items-center gap-10">
        <h2 id="test-drive-steps-heading" className="text-center text-[clamp(1.25rem,0.4167vw+1.1667rem,1.5rem)] leading-[1.2] font-bold text-texto">
          {steps.headingPrefix}
          <span className="text-azul-base">{steps.headingHighlight}</span>
        </h2>

        <Reveal className="w-full">
          <ol className="grid w-full grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
            {howItWorksSteps.map((step, index) => {
              const text = steps.items[index];
              return (
                <li
                  key={step.number}
                ...
                >
                  <div className="flex w-full items-center gap-[15px]">
                    ...
                    <p className="flex-1 text-[clamp(1.0625rem,0.1042vw+1.0417rem,1.125rem)] font-bold leading-[1.2] text-texto">{text.title}</p>
                    <span
                  ...
                  <p className="text-sm leading-[1.2] font-medium text-texto">{text.description}</p>
                </li>
              );
            })}
          </ol>
