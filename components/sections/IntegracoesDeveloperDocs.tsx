...
export default async function IntegracoesDeveloperDocs() {
  ...
  return (
    <section aria-label={docsCard.heading} className="flex justify-center bg-branco px-5 py-10">
      <div className="grid w-full max-w-[1400px] grid-cols-1 gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,350px)]">
        <Reveal
          className="flex flex-col items-center gap-5 rounded-[20px] border border-contorno-base px-5 py-[30px] lg:min-h-[310px]"
        ...
        >
          ...
          {/* Below lg: Blocos Mobile — each language becomes its own bordered card. "Complete
              Box" (see lib/completeBox.ts) keeps the wrap balanced instead of a short last
              row leaving a gap. */}
          <ul aria-label={docsCard.langsAriaLabel} className="flex w-full flex-wrap gap-2.5 lg:hidden">
            {integracoesDevLangs.map((lang) => (
              <li
                key={lang.label}
                className={`flex grow min-w-[80px] ${getCompleteBoxBasis(integracoesDevLangs.length)} flex-col items-center gap-[15px] rounded-[14px] border border-contorno-base bg-branco p-4`}
              >
          ...
          <ul aria-label={docsCard.langsAriaLabel} className="hidden w-full flex-wrap items-start justify-center gap-2.5 lg:flex">
            {integracoesDevLangs.map((lang) => (
              <li key={lang.label} className="flex min-w-0 grow basis-[calc(20%-0.5rem)] flex-col items-center gap-[15px]">
                <Image src={lang.icon} alt="" aria-hidden="true" width={40} height={40} className="shrink-0" />
                <p className="w-full text-center text-xs leading-[1.2] font-bold text-texto">{lang.label}</p>
              </li>
            ))}
        ...
        <Reveal className="flex flex-col items-start gap-5 rounded-[20px] bg-gradient-to-r from-[#001d6b] to-[#000928] px-5 py-[30px] lg:min-h-[310px]">
          ...
          <ApiCodeSample fileName={codeCard.sampleFileName} tags={codeCard.sampleTags} />
        </Reveal>
        ...
        <Reveal className="relative flex flex-col justify-between gap-[30px] overflow-hidden rounded-[20px] px-5 py-[30px] lg:min-h-[310px]">
          <Image src={PLANO_AZUL_BASE} alt="" aria-hidden="true" fill sizes="(min-width: 1024px) 33vw, 100vw" className="-z-10 object-cover" />
